    ...
    <div className="hidden w-full items-center justify-between bg-white px-4 py-4 lg:flex">
      ...
      <div className="flex items-center justify-center gap-6">
        {navbarContent?.Links.map((item, index) => {
          ...
          const className = `text-base font-medium ${item.isPrimary ? 'text-cst-primary' : 'text-gray-900'} ${item.is_cta ? 'hover:text-cst-secondary flex items-center gap-2 rounded-full border bg-black px-6 py-2 text-white transition-all duration-500 hover:border-black hover:bg-white' : 'hover:text-cst-primary'} transition-colors`
