...
import { FooterLink, FooterResponse } from '@/types/footer'
...
const FooterClient = ({ content }: any) => {
  const { lang } = useContext(LanguageContext)

  const { data, isLoading } = useQuery<FooterResponse>({
    queryKey: ['footer', lang.code],
    initialData: lang.code === 'en' ? content : undefined,
    queryFn: async () => {
      const res = await axios.get(`/api/footer?lang=${lang.code}`)
      return res.data
    },
  })
      ...
      <footer className="relative w-full overflow-hidden border-t border-slate-200 bg-slate-50 pt-24 pb-8">
        <div className="relative z-10 mx-auto mb-24 max-w-[2000px] px-4 md:px-8">
          <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-12">
            {/* Left Big Box */}
            <div className="flex md:col-span-12 lg:col-span-7">
              <div className="flex h-full min-h-[400px] w-full flex-col justify-between rounded-md bg-white p-8 sm:p-12">
                <div>
                  <Skeleton className="mb-6 h-12 w-48" />
                  <Skeleton className="h-6 w-full max-w-md" />
                  <Skeleton className="mt-2 h-6 w-3/4" />
                </div>
                <Skeleton className="mt-16 h-8 w-52 rounded-full" />
              </div>
            </div>
            ...
            <div className="grid h-full grid-cols-1 gap-6 sm:grid-cols-2 md:col-span-12 lg:col-span-5">
              ...
              <div className="flex w-full">
                <div className="flex w-full flex-col rounded-md bg-white p-10">
            ...
            <div className="md:col-span-12">
              <div className="flex w-full flex-col items-center justify-between rounded-md bg-white px-10 py-6 sm:flex-row">
  ...
  return (
    <footer className="relative w-full overflow-hidden border-t border-slate-200 bg-slate-50 pt-24 pb-8">
          ...
          >
            <div className="group relative flex h-full min-h-[400px] w-full flex-col justify-between overflow-hidden rounded-md bg-white p-8 sm:p-12">
              ...
              <div className="relative z-10">
                ...
                >
                  {getStrapiUrl(footer?.logo?.url) && (
                    <Image
                      src={getStrapiUrl(footer?.logo?.url)!}
                      alt={footer?.logo?.alternativeText || 'Footer Logo'}
                      width={48}
                      height={48}
          ...
          <div className="grid h-full grid-cols-1 gap-6 sm:grid-cols-2 md:col-span-12 lg:col-span-5">
            ...
            <FadeContent blur delay={200} duration={800} className="flex w-full">
              <div className="flex w-full flex-col rounded-md bg-white p-10 transition-all hover:border-teal-200">
                ...
                <nav className="flex flex-col gap-6">
                  {footer?.Links?.slice(0, 2)?.map((link: FooterLink) => (
                    <Link
                      key={link.id}
            ...
            <FadeContent blur delay={300} duration={800} className="flex w-full">
              <div className="flex w-full flex-col rounded-md bg-white p-10 transition-all hover:border-blue-200">
                ...
                <nav className="flex flex-col gap-6">
                  {footer?.Links?.slice(2)?.map((link: FooterLink) => (
                    <Link
                      key={link.id}
                      href={link.href === 'home' ? '/' : link.href}
                      className="w-max text-lg font-medium text-slate-600 transition-all hover:translate-x-2 hover:text-slate-900"
              ...
              <div className="mt-4 flex gap-8 sm:mt-0">
                {footer?.social_links?.map((social) => (
                  <a
                    ...
                    className="text-slate-600 transition-all hover:text-slate-900"
                  >
                    {social.platform}
                  </a>
                ))}
      ...
      <div className="pointer-events-none relative z-0 mt-[-8vw] flex w-full items-end justify-center overflow-hidden">
        ...
        >
          {getStrapiUrl(footer?.footer_logo?.url) && (
            <Image
              src={getStrapiUrl(footer?.footer_logo?.url)!}
              alt={footer?.footer_logo?.alternativeText || 'Footer Logo'}
              width={48}
              height={48}
              className="inline-block h-auto w-full max-w-full opacity-10"
            />
          )}
        </h1>
      </div>
